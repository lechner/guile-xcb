  ...
  (sxml-match exp
    ...
    ((import ,import) (guard (string? import))
     (if (not (string= "xproto" import))
         (make-element-syntax 'import `(use-modules (xcb xml ,(string->symbol import))) #f)
       ...
       (make-element-syntax
        ...
        `(begin
           (use-modules
            (srfi srfi-1)
            (rnrs bytevectors)
            (language xml-xcb type)
            (language xml-xcb struct)
            (language xml-xcb union)
            (language xml-xcb enum)
            (language xml-xcb support)
            (xcb connection)
            (xcb doc)
            ,@(if (not (eq? header-symbol 'xproto))
                  '((xcb xml xproto)
                    (xcb extension-support)) '()))
